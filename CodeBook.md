...
## About variables:   
* `x_train`, `y_train`, `x_tst`, `y_tst`, `sub_train` and `sub_tst` contain the data from the downloaded files.
* `all_train`, `all_test` and `finaldata` merge the previous datasets to further analysis.
* `features` contains the correct names for the `finaldata` dataset, which are applied to the column names stored in.
